...
class Group_Tabular(Group_TabularTemplate):
    def __init__(self, **properties):
        ...
        try:
            self.tabulator.data = anvil.server.call("get_list_data", n=100)
        ...
        def delete_link_formatter(cell, **params):
            l = Link(icon="fa:trash", foreground="indianred", spacing_above="none", spacing_below="None")
            return l
        ...
        self.tabulator.columns = [
            ...
            {"title": "Gender", "field": "gender", "editor": True},
...
#             "selectable_persistence": False,
        ...
        self.columns_dropdown.items = [col["field"] for col in self.tabulator.columns[1:-1]]
        self.fields_dropdown.items = [col["field"] for col in self.tabulator.columns[1:-1]]
    ...
    def sort_button_click(self, **event_args):
        ...
        self.tabulator.set_sort(col, "asc" if asc else "desc")

    def reset_sort_click(self, **event_args):
        """This method is called when the button is clicked"""
        self.tabulator.clear_sort()
    ...
    def do_delete(self, data):
        ...
        print(c)
        if c:
            self.tabulator.delete_row(data["id"])

    def tabulator_cell_click(self, cell, **event_args):
    ...
    def add_row_button_click(self, **event_args):
        ...
        if self.tabulator.data:
            row["id"] = max(row["id"] for row in self.tabulator.data) + 1
        print("row with id", row["id"], "being added")
        self.tabulator.add_row(row, True)

    # easily delete rows
    def delete_button_click(self, **event_args):
        """This method is called when the button is clicked"""
        data = self.tabulator.get_selected_data()
        if not data:
            return
        ...
        self.tabulator.delete_row(indices)
        anvil.js.call("loadSpinner", False)

    def tabulator_page_loaded(self, pageno, **event_args):
        """This method is called when a page is loaded"""
        print("loaded", pageno)

    def set_locale(self, sender, **event_args):
        """This method is called when the link is clicked"""
        self.tabulator.set_locale(sender.tag)
        print(f"setting locale to {sender.tag}")

    def tabulator_row_formatter(self, row, **event_args):
    ...
    def tabulator_row_selection_changed(self, rows, data, **event_args):
